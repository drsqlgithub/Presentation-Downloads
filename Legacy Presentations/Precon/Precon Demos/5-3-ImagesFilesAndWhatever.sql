...
if exists (select * from sys.databases where name = 'FileStorageDemo')
 exec ('
alter database  FileStorageDemo
	set single_user with rollback immediate;

drop database FileStorageDemo;')
...
GO
/*
Msg 5505, Level 16, State 1, Line 2
...
--now we will configure filetable
EXEC sp_configure filestream_access_level, 2;
RECONFIGURE;
GO
--gset up non-transacted access and the name of the database dir for the UNC name
ALTER database FileStorageDemo
	SET FILESTREAM (NON_TRANSACTED_ACCESS = FULL, 
                         DIRECTORY_NAME = N'RelationalDesignPrecon');
