...
WITH EXECUTE AS SELF --must be able to authenticate the owner of the object.
					 --or it will not work. comment out if you are DBO and cannot execute
AS
...
--with * it still looks normal...but there are other ways!
select *
from   Hardware.Equipment;
...
select count(*)
FROM   Hardware.Equipment
group by HammerHeadStyle

select *
from   Hardware.Equipment;

--can predict XML format using
SELECT name
FROM   sys.columns
WHERE  OBJECT_NAME(object_id) = 'Equipment'
and is_sparse = 1
