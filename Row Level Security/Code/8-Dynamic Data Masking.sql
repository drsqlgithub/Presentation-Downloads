...
GO
------------------------------------------------------------------
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'TestDynamicDataMasking')
	ALTER DATABASE TestDynamicDataMasking SET  SINGLE_USER WITH ROLLBACK IMMEDIATE
GO
USE master
GO
DROP DATABASE IF EXISTS TestDynamicDataMasking
GO
...
    InceptionTime DATETIME2(0) NOT NULL, --Time we first saw this person. Usually the row create time, but not always 
...
ALTER TABLE Demo.Person ALTER COLUMN StatusCode  --just replacing all of the value
    ADD MASKED WITH (Function = 'partial(0,"Nunya",0)'); 
GO
...
       MIN(PersonNumber) as PersonNumber,MIN(EmailAddress) AS EmailAddress, COUNT(*) as MatchingRows 
FROM   Demo.Person;
...
--no effect, other than the data is masked... So you could pretty easily determine what data is in the table if you gave it a try.
...
CREATE OR ALTER PROCEDURE dbo.Person$select
WITH EXECUTE AS 'dbo'
AS
    SELECT *
    FROM   demo.Person;
...
--Give NormalUser rights to data:

GRANT UNMASK TO NormalUser; --Database level only, all unmasked,or none.
GO
